"""2018 NAS Matching Script.

Matches and exports 2018 NAS files:
- WSOP 2018 Main Event: 16 files (Episodes + Final Table)
"""
import sys
import io
import re
...
def is_final_table(filename: str) -> bool:
    return 'Final Table' in filename


def generate_entry_key(elem: NasElement) -> str:
    parts = ['WSOP_2018_ME']
    if elem.is_final_table:
        parts.append('FT')
    elif elem.episode_num:
...
def export_to_sheets(elements: list[NasElement]):
    ...
    # Sort: Episodes first by number, Final Table last
    def sort_key(x):
        if x.is_final_table:
            return (1, 999)
        return (0, x.episode_num or 0)
    ...
    for idx, elem in enumerate(sorted(elements, key=sort_key), 1):
        ...
        day = 'Final Table' if elem.is_final_table else ''

        rows.append([
            idx, entry_key, 'NAS_ONLY', elem.role, '-',
            'WSOP 2018 Main Event', title, '',
            elem.region, elem.event_type, '', day, '', '',
            f'{size_gb:.2f}', elem.filename, elem.full_path
        ])

    write_sheet(sheets, '2018_Catalog', rows)
...
def main():
    ...
    print('\n' + '=' * 70)
    print('[OK] 2018 Matching completed!')
    print('=' * 70)
