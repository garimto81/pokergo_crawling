"""Search specific titles in GG sheet and Iconik.

특정 title들의 모든 정보를 조회합니다.
"""
...
def search_in_gg_sheet():
    """Search titles in GG sheet."""
    print("=" * 80)
    print("GGmetadata_and_timestamps 시트에서 검색")
    print("=" * 80)

    writer = SheetsWriter()
    headers, data = writer.get_sheet_data("GGmetadata_and_timestamps")

    print(f"\n총 {len(data)}건 중 검색...")
    print(f"Headers: {headers[:10]}...")  # 처음 10개 헤더만
    print()
    ...
    for target in TARGET_TITLES:
        ...
        if matches:
            ...
            for m in matches:
                ...
                # Print all available fields
                for key, val in m.items():
                    if val and key not in ['title', 'id', 'time_start_ms', 'time_end_ms']:
                        print(f"  {key}: {val}")
                print()
        else:
            print("  -> 매칭 결과 없음")
